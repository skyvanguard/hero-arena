...
**Status: IN PROGRESS (round 11).** **LAN discovery shipped:** dedicated UDP discovery port (7778) carrying M_DISCOVER_PING/REPLY (live open/full/over state, team size, human count, game port); SCAN button in hero-select broadcasts the ping and also unicasts it to the explicit host in the join field (that leg reaches the host across the emulator NAT, where broadcasts do not), with tap-to-fill result rows; 6-check discovery suite green. **Round 10: Net v1.1** — server input sanitization + wrap-aware seq gate (move/aim clamps, stale frames and stale edges dropped), session-token reconnect (slot freeze, token re-hello, same CharacterEntity; client bounded retry), **World lag-comp hitscan** (60 Hz pose history, 200 ms analytic 3-sphere rewind; weapons now share the single world.hitscan() truth, rays include walls), **client prediction** (private world + twin driven by the real NetPlayerController from the same 20 Hz samples; hard-snap reconciliation above 0.35 m; own view follows the twin), and the **SimLink** net-sim transport (per-direction latency/loss) with a 150 ms RTT + 2% loss full-stack suite. **205/205 headless checks green, 0 script errors** (10 suites). Subtle bugs found by the new suites and fixed: (1) two exactly-coincident capsules push each other up at about 66 m/s in 4.7 — a human could spawn on the live bot at its spawn point (server now yields the bot AT the spawn, immediate free); (2) a freed shooter left in a live projectile aborted snapshot packing (guard added); (3) the reattach test flaked when the match ended first (test world score cap raised); (4) Godot 4.7 removed `set_mode_server`/`set_mode_client` — discovery uses `bind()` + `get_available_packet_count()` (read packet ip/port only after `get_packet()`). Remaining for exit: two-phone LAN verification, internet play + matchmaking prototype, docker server ops + 2-core budget, more latency/loss profiles.
