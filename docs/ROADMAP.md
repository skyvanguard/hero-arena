...
- [ ] Heroes #2–#6 (2 Assault, 1 Tank, 2 Support/Utility, 1 Controller per directive §6) — each with distinct movement/combat identity. **Progress: #2 Blitz (Assault/Sprint, projectile) + #3 Bastion (Tank/Armor, pellet shotgun) built & tested**; Mira/Patch/Nimbus pending
...
- [x] Roster tests: per-hero kit validation + DPS/TTK balance table with role bands (`tests/test_roster.tscn`, 3 heroes green). CC-lock + ult-charge pacing checks extend it as #4–#6 land.
