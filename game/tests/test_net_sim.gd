...
func _run() -> void:
	# 1: connect + slot through the lossy link (reliable channel + 150 ms
	# RTT; bounded wait: a lost slot reply is recovered by the D30
	# re-hello watchdog, so give it time).
	var cw := 0
	...
	check("sim: lost slot reply -> re-hello re-announces the same character",
